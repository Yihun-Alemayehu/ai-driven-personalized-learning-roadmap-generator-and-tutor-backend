// ─── RoadmapCanvas ────────────────────────────────────────────────────────────
// Wraps @xyflow/react. Converts backend nodes/edges → React Flow format,
// runs dagre auto-layout when positions are absent, and fires onNodeClick
// for unlocked nodes so the parent can open the detail drawer.
//
// deps: npm install @xyflow/react dagre @types/dagre
...
import type { RoadmapNode, RoadmapEdge } from '../roadmap.types';
...
// Register custom node type — define OUTSIDE the component to prevent re-registration.
...
function autoLayout(nodes: Node[], edges: Edge[]): Node[] {
  ...
  nodes.forEach((n) => {
    const isBranch = (n.data as RoadmapNode).isBranchingPoint;
...
function toFlowNodes(roadmapNodes: RoadmapNode[]): Node[] {
  return roadmapNodes.map((n) => ({
    ...
    data: n,
    // Prevent React Flow from allowing drag on locked nodes
    draggable: false,
...
export function RoadmapCanvas({ roadmapNodes, roadmapEdges, onNodeClick }: RoadmapCanvasProps) {
  const nodeMap = useMemo(
    () => new Map(roadmapNodes.map((n) => [n.id, n])),
    [roadmapNodes]
  );

  // Derive RF nodes — run auto-layout if positions absent
    ...
    <div className="flex-1 h-full" style={{ background: '#f3efe7' }}>
      ...
      >
        ...
        <Controls
          ...
          className="!shadow-none !border !border-[#d6cfbf] !rounded-lg !overflow-hidden"
          style={{ background: '#faf7f1' }}
        />
        <MiniMap
          nodeColor={(n) => {
            const state = (n.data as RoadmapNode)?.masteryState;
            const colorMap: Record<string, string> = {
              mastered:      '#60a870',
              in_progress:   '#4a7fc1',
              review_needed: '#c9a030',
              not_started:   '#b0a890',
              relearn:       '#c8613a',
              locked:        '#d0c8b8',
            };
            return colorMap[state] ?? '#ccc';
          }}
          maskColor="rgba(243,239,231,0.6)"
          className="!rounded-lg !border !border-[#d6cfbf]"
          style={{ background: '#faf7f1' }}
        />
      </ReactFlow>
    </div>
