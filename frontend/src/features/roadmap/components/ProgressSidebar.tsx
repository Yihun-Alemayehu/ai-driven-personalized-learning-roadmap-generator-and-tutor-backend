...
function Divider() {
  return <hr className="border-none" style={{ borderTop: '1px solid #d6cfbf', margin: 0 }} />;
}
...
export function ProgressSidebar({
  ...
  return (
    <aside
      className="w-[252px] flex-shrink-0 flex flex-col gap-[22px] overflow-y-auto overflow-x-hidden px-5 py-[22px]"
      style={{ background: '#faf7f1', borderRight: '1px solid #d6cfbf' }}
    >
      {/* Domain */}
      <div className="flex flex-col gap-2.5">
        <SectionLabel>currently enrolled</SectionLabel>
        <h2 className="text-[22px] font-medium leading-[1.05] tracking-[-0.01em] m-0"
          style={{ fontFamily: "'Cormorant Garamond', serif", color: '#1a1614' }}>
          {domainName}
        </h2>
        <div className="text-[13px]" style={{ color: '#6e645a', fontFamily: "'Crimson Pro', serif" }}>
          {enrolledAt}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2.5">
        ...
        <div>
          <div className="flex justify-between items-baseline mb-2">
            ...
            <span className="text-[11px]" style={{ fontFamily: 'JetBrains Mono, monospace', color: '#6e645a' }}>
              {stats.masteredCount} / {stats.totalNodes}
            </span>
          </div>
          <div className="h-[7px] rounded-full overflow-hidden" style={{ background: '#ebe6db' }}>
            <div className="h-full rounded-full transition-[width] duration-500"
              style={{ width: `${stats.completionPercent}%`, background: 'oklch(0.62 0.18 28)' }} />
          </div>
        </div>
      </div>
          ...
          {MASTERY_ROWS.map(({ state, label }) => {
            ...
            return (
              <div key={state} className="flex items-center gap-2 text-[13px]" style={{ color: '#3a342e' }}>
                <span className="w-2.5 h-2.5 rounded-[3px] flex-shrink-0 border"
                  style={{
                    background: cfg.dotColor,
                    borderColor: `color-mix(in srgb, ${cfg.dotColor} 40%, transparent)`,
                  }} />
                <span className="flex-1" style={{ fontFamily: "'Crimson Pro', serif" }}>{label}</span>
                <span className="text-[11px] font-semibold" style={{ fontFamily: 'JetBrains Mono, monospace', color: '#1a1614' }}>
                  {count}
                </span>
              </div>
        ...
        <div className="flex gap-1">
          {BRANCH_PATHS.map(({ path, label }) => (
            <button
              ...
              className="flex-1 py-1.5 text-center rounded-lg border text-[10.5px] tracking-[0.04em] transition-all duration-150"
              style={{
                fontFamily: 'JetBrains Mono, monospace',
      ...
      {nextNode && (
        <>
          ...
          <div className="flex flex-col gap-2.5">
            ...
            <div className="border rounded-[10px] p-3.5" style={{ background: '#f3efe7', borderColor: '#d6cfbf' }}>
              <div className="text-[18px] font-medium mb-1" style={{ fontFamily: "'Cormorant Garamond', serif", color: '#1a1614' }}>
                {nextNode.title}
              </div>
              <div className="text-[13px] mb-2.5" style={{ color: '#6e645a', fontFamily: "'Crimson Pro', serif" }}>
                {nextNode.estimatedHours != null ? `${nextNode.estimatedHours}h · ` : ''}
