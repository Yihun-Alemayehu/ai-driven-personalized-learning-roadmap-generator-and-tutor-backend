...
export default function AttemptReviewPage() {
  ...
  if (isLoading) {
    return (
      <div className="flex h-screen items-center justify-center" style={{ background: pageBackground }}>
        <span className="font-mono text-xs tracking-widest animate-pulse" style={{ color: '#9a9088' }}>loading review…</span>
      </div>
    );
  }
  ...
  if (isError || !attempt) {
    return (
      <div className="flex h-screen items-center justify-center flex-col gap-4" style={{ background: pageBackground }}>
        <p style={{ fontFamily: "'Crimson Pro', serif", color: '#6e645a' }}>Attempt not found.</p>
  ...
  const submittedMap = new Map(
    (attempt.answers as { questionId: string; answer: string }[]).map((a) => [a.questionId, a.answer]),
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <nav className="flex items-center gap-1.5 text-[13px]" style={{ fontFamily: 'JetBrains Mono, monospace', color: '#6e645a' }}>
      ...
      <div className="flex-1 px-6 py-8 max-w-2xl mx-auto w-full">
        {/* Header */}
        <div className="mb-6">
          <div className="text-[12px] tracking-[0.12em] uppercase mb-1" style={{ fontFamily: 'JetBrains Mono, monospace', color: '#9a9088' }}>
            attempt review
          </div>
          <h1 className="text-[30px] font-medium leading-tight" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
            {attempt.node.title}
          </h1>
          <div className="flex items-center gap-4 mt-2 text-[13px]" style={{ color: '#6e645a', fontFamily: 'JetBrains Mono, monospace' }}>
            <span>{scorePercent}% · {correct}/{attempt.quiz.questions.length} correct</span>
            <span>·</span>
            ...
            <span
              ...
              style={{
                borderColor: '#d6cfbf',
                color: attempt.outcome.includes('pass') ? 'oklch(0.60 0.13 150)' : 'oklch(0.60 0.18 28)',
              }}
            >
        ...
        <div className="flex gap-3 mt-8">
          <Button
            ...
            style={{ background: '#1a1614', color: '#f3efe7', fontFamily: "'Crimson Pro', serif", fontSize: 15 }}
            onClick={() => navigate(-1)}
          >
            ← Back
          </Button>
