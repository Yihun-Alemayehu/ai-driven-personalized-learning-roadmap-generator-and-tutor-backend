...
export function AiAnalysisPanel({ domainId, domainName }: Props) {
  const { state, text, generate, regenerate, reset } =
    useAnalyticsAiStream(domainId);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          borderColor: "color-mix(in srgb, oklch(0.55 0.13 250) 20%, #faf7f1)",
          background: "color-mix(in srgb, oklch(0.55 0.13 250) 6%, #faf7f1)",
        }}
      >
      ...
      {/* Body — max-width keeps lines short enough to read comfortably */}
      <div className="px-5 py-4 max-w-4xl">
        {state === "idle" && (
          <div className="flex flex-col items-center gap-3 py-8 text-center">
            ...
            >
              The AI will analyse learner performance data for{" "}
              <strong>{domainName}</strong> and return a situation summary, key
              concerns, and specific curriculum recommendations.
            </p>
          </div>
        )}
        ...
        {state === "error" && (
          <p
            className="text-[13px] py-4 text-center italic"
            style={{ fontFamily: "'Crimson Pro', serif", color: "#9a9088" }}
          >
            Could not generate analysis. Check that the AI service is running
            and try again.
          </p>
        )}
      </div>
