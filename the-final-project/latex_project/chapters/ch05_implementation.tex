...
\begin{itemize}
    ...
    \item \textbf{SSE proxy pattern:} For streaming endpoints, the learning-service pipes the ai-service SSE response directly to the client without buffering, using Node.js \texttt{ReadableStream} forwarding. Nginx SSE location blocks ensure \texttt{proxy\_buffering off} and disable gzip on stream routes.
...
Both AI explanation and AI Instructor chat use identical SSE scaffolding: headers are set and flushed before generation starts, each token is emitted as an SSE event, a \texttt{[DONE]} sentinel signals completion, and an abort controller honours client disconnection.

Source: \texttt{backend/services/ai-service/src/modules/ai/ai.controller.ts}

\begin{lstlisting}[language=JavaScript]
export async function streamExplanation(
  req: Request, res: Response,
): Promise<void> {
  // X-Accel-Buffering: no tells Nginx not to accumulate tokens before forwarding
  ...
  req.on('close', () => controller.abort());    // honour client disconnect

  await svc.streamExplanation(req.body, sendChunk, controller.signal);
...
\begin{itemize}
    ...
    \item \textbf{SSE buffering:} Nginx and Node.js compression middleware both buffered SSE tokens before delivery, producing batched outputs instead of live streaming. The fix was adding \texttt{X-Accel-Buffering: no} response headers and a compression filter that skips routes containing \texttt{/stream}.
